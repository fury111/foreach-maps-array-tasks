...
function showFirstAndLast(arr) {
  const letters = [];
  arr.forEach((str) => {
    letters.push(str[0] + str[str.length - 1]);
  });
  return letters;
}
...
function addKeyAndValue_2(arr, key, value) {
  arr.forEach((obj, index) => {
    arr[index] = { ...obj, [key]: value };
  });
  return arr;
}
console.log(
...
function vowelCount(str) {
  ...
  [...str].forEach((letter) => {
    if (vowels.includes(letter)) {
      vowel[letter] = (vowel[letter] || 0) + 1;
    }
  });
  
  return vowel;
}
console.log(vowelCount("hello tasdlf asdhflasd asdfh")); 